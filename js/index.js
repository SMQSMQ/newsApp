...
sinaNews.factory('$type',[function(){
  var typeList = [
    {name: '头条',state: true},
    {name: '视频',state: true},
    {name: '娱乐',state: true},
...
sinaNews.controller('headerController',['$scope','$type',function($scope,$type){
  $scope.typeList = $type.getAllType();
}])
...
sinaNews.directive('uqYindao',[function(){
  return {
    ...
    link: function($scope,elem){
      if(localStorage.sinaX){
        $scope.isFirstOpen = false;
      }else{
        $scope.isFirstOpen = true;
      }
      $scope.enter = function(){
        localStorage.sinaX = true;
        $scope.isFirstOpen = false;
      }
    }
  }
}]).directive('uqSinaHeader',[function(){
  return {
    replace: true,
    restrict: 'EAC',
    templateUrl:'views/sina-header.html'
  }
}]).directive('uqSinaFooter',[function(){
  return {
    replace: true,
    restrict: 'EAC',
    templateUrl:'views/sina-footer.html'
  }
}]).directive('uqType',[function(){
  return {
    replace: true,
    restrict: 'EAC',
